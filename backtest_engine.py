...
class BacktestEngine:
    ...
    async def run_backtest(
        ...
        for i in range(start_idx, len(df)):
            ...
            if self.positions:
                ...
                can_trade = (
                    abs(decision["signal"]) > 0.20 and  # 信号强度>5%
                    decision["confidence"] > 0.50  # 置信度>30%
                )
                ...
                if can_trade:
                    ...
                    self._enter_position(
                        current_price, 
                        current_time, 
                        decision['leverage'] if decision['leverage'] > 0 else 5,  # 默认8倍杠杆
                        decision['confidence'],
                        side
                    )
    ...
    def _enter_position(
        ...
        """开仓"""
        # 计算仓位大小
        position_size = self.capital * leverage * 0.30  # 使用50%资金（更保守）
        contracts = position_size / price
        
        # 计算止损价（根据方向）
        if side == 'long':
            stop_loss = price * (1 - 0.015 / leverage)
        else:
            stop_loss = price * (1 + 0.015 / leverage)
